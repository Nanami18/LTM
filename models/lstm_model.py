...
def build_model(cfg, obs_space, action_space):
    if cfg.use_embed:
        ...
        elif cfg.use_ext_mem:
            assert(cfg.use_memory is False)
            model = ACModelWithLinearProcsAndExternalMemory(obs_space, action_space,
                False, cfg.use_text, cfg.token_embed_size, cfg.image_embed_size)
        else:
...
class ACModelWithLinearProcsAndExternalMemory(nn.Module, torch_ac.RecurrentACModel):
    def __init__(self, obs_space, action_space, use_memory=False, use_text=False, embed_size=16, image_embed_size=128):
        super().__init__()
        ...
        self.memory_embed = nn.Embedding(3, 4)
        ...
        self.actor = nn.Sequential(
            nn.Linear(self.embedding_size, self.embedding_size),
            nn.Tanh(),
            nn.Linear(self.embedding_size, action_space[0].n)
        )
        ...
        self.memory_writting = nn.Sequential(
            nn.Linear(self.embedding_size, self.embedding_size),
            nn.Tanh(),
    ...
    def forward(self, obs, memory):
        ...
        if self.use_memory:
            ...
            embedding = x
            memory_embed = self.memory_embed(obs.memory)
            embedding = torch.cat((embedding, memory_embed), dim=1)
        ...
        x = self.actor(embedding)
        dist = Categorical(logits=F.log_softmax(x, dim=1))
        memory_w = self.memory_writting(embedding)
        memory_w = Categorical(logits=F.log_softmax(memory_w, dim=1))
        x = self.critic(embedding)
        value = x.squeeze(1)
        return dist, value, memory_w
